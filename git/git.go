...
func parseCommit(commit string, commits []*Commit) []*Commit {
	...
	newCommit := &Commit{}
	newCommit.Hash = strings.Replace(lines[0], "\n", "", 0)
	newCommit.Author = strings.Replace(lines[2], "\n", "", 0)
	newCommit.Date = strings.Replace(lines[1], "\n", "", 0)
	newCommit.Subject = strings.Replace(lines[3], "\n", "", 0)
